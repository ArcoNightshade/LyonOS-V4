{ pkgs, settings, ... }:
{
  imports = [
    ...
    # Illumix: shared Latitude 5420 hardware quirks, plus the headless
    # illumos guest and its SSH-backed terminal. See modules/virt/illumix.nix
    # for guest one-time setup instructions.
    ../../modules/hardware/latitude-5420.nix
    ../../modules/virt/illumix.nix
    ../../modules/virt/illumix-terminal.nix
    ../../modules/net/illumix-nat.nix
  ];
  ...
  # (Latitude-5420-specific PCIe/NVMe/amdgpu params come from latitude-5420.nix above.)
  boot.kernelParams = [
    "splash"
    "quiet"
  ...
  # ---- Illumix guest + terminal ----
  illumix.enable = true;
  illumix.terminal.guestUser = settings.account.name;

  # ---- Operator remote access (host machine, not the guest) ----
  services.openssh.enable = true;
  # Placeholder -- replace with the operator's real public key before
  # deploying. As written this won't match any private key.
  users.users.${settings.account.name}.openssh.authorizedKeys.keys = [
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIREPLACE-WITH-YOUR-REAL-KEY operator@illumix"
  ];
}
